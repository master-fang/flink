...
import com.fang.traffic.constant.{AvgSpeedInfo, DeckCarWarning, OutOfLimitSpeedInfo}
import org.apache.flink.configuration.Configuration
import org.apache.flink.streaming.api.functions.sink.{RichSinkFunction, SinkFunction}

class MysqlWriteDataSink[T](classType: Class[_ <: T]) extends RichSinkFunction[T] {
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    if (classType.getName.equals(classOf[DeckCarWarning].getName)) {
  ...
  override def invoke(value: T, context: SinkFunction.Context[_]): Unit = {
    ...
    if (classType.getName.equals(classOf[DeckCarWarning].getName)) {
      val info: DeckCarWarning = value.asInstanceOf[DeckCarWarning]
