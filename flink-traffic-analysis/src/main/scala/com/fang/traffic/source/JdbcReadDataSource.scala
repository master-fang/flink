package com.fang.traffic.source

import java.sql.{Connection, DriverManager, PreparedStatement, ResultSet}

import com.fang.traffic.constant.{MonitorInfo, ViolationInfo}
...
class JdbcReadDataSource[T](classType: Class[_ <: T]) extends RichSourceFunction[T] {
  ...
  override def run(ctx: SourceFunction.SourceContext[T]): Unit = {
    while (flag) {
      ...
      while (set.next()) {
        if (classType.getName.equals(classOf[MonitorInfo].getName)) {
          var info = MonitorInfo(set.getString(1), set.getString(2), set.getInt(3), set.getString(4))
